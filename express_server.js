...
app.get('/api/v1/papers/:id', (request, response) => {
  database('papers').where('id', request.params.id).select() // investigate single return instead of where
  .then(papers => {
    if (papers.length) {
      response.status(200).json(papers[0]); // added [0] to avoid array return
    } else {
